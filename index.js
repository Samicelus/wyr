						...
						if(typeof(data)!= "undefined"){
							...
							if(typeof(obj)!= "undefined"){
								...
								//商户登录
									...
									var openid = "";
									var userType = "";
									var xjwt = request.headers['x-json-web-token'];
									if(xjwt){
										jwtHandler.checkJwt(xjwt,db,function(result){
											//console.log('jwtRet: '+result);
											if(!result){
												console.log('验证不通过!');
												var ret = JSON.stringify({result:'error',msg:'用户信息验证失败'});
												httpRet.writeResponse(response,ret);
												}else{
													console.log('验证通过!openid: '+result.openid+"  userType: "+result.userType);
